...
curs = exec(conn, 'SELECT * FROM u_equivital_1');
curs = fetch(curs);
data = curs.Data;
y0          = cell2mat(data(:,2));
u0_bolo     = cell2mat(data(:,3));
u0_meal     = cell2mat(data(:,4));
u0_HR       = cell2mat(data(:,5));
u0_BR       = cell2mat(data(:,6));
u0_temp     = cell2mat(data(:,7));
u0_RR       = cell2mat(data(:,8));
u0_BW       = cell2mat(data(:,9));
u0_lat_acc  = cell2mat(data(:,10));
u0_long_acc = cell2mat(data(:,11));
u0_ver_acc  = cell2mat(data(:,12));

curs = exec(conn, 'SELECT * FROM u_equivital_2');
curs = fetch(curs);
data = curs.Data;
y1          = cell2mat(data(:,2));
u1_bolo     = cell2mat(data(:,3));
u1_meal     = cell2mat(data(:,4));
u1_HR       = cell2mat(data(:,5));
u1_BR       = cell2mat(data(:,6));
u1_temp     = cell2mat(data(:,7));
u1_RR       = cell2mat(data(:,8));
u1_BW       = cell2mat(data(:,9));
u1_lat_acc  = cell2mat(data(:,10));
u1_long_acc = cell2mat(data(:,11));
u1_ver_acc  = cell2mat(data(:,12));

curs = exec(conn, 'SELECT * FROM u_equivital_3');
curs = fetch(curs);
data = curs.Data;
y2          = cell2mat(data(:,2));
u2_bolo     = cell2mat(data(:,3));
u2_meal     = cell2mat(data(:,4));
u2_HR       = cell2mat(data(:,5));
u2_BR       = cell2mat(data(:,6));
u2_temp     = cell2mat(data(:,7));
u2_RR       = cell2mat(data(:,8));
u2_BW       = cell2mat(data(:,9));
u2_lat_acc  = cell2mat(data(:,10));
u2_long_acc = cell2mat(data(:,11));
u2_ver_acc  = cell2mat(data(:,12));
...
table_name = 'rmse_ARX_2_y0_equivital';       % Tabla (1 para cada conjunto de y
new_columns = { 'train_bolo_meal_todo',...
                 'test_bolo_meal_todo'}; % Columnas
...
data_train = [y_train, u1_train, u2_train, u3_train, u4_train, ...
    u5_train, u6_train, u7_train, u8_train, u9_train];
fprintf('N de train: %i\n', length(data_train))

% Extraemos los datos del testing
data_test = [y_test, u1_test, u2_test, u3_test, u4_test,...
    u5_test, u6_test, u7_test, u8_test, u9_test];
fprintf('N de test:  %i\n', length(data_test))
...
for n=1:12
    ...
    theta = [A B(1,:) B(2,:) B(3,:) B(4,:) B(5,:) B(6,:) B(7,:) B(8,:)...
        B(9,:)];      
    theta = theta'; % Por definicion es vector fila

    % Realizamos las predicciones con una funcion dedicada
    Y_hat_train = ARX_k_stepV4(data_train, theta, [na max(nb)], k);
    Y_hat_test  = ARX_k_stepV4(data_test,  theta, [na max(nb)], k);
