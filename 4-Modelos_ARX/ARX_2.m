...
% Pedimos y separamos los datos de u de training para bolo
...
% Pedimos y separamos los datos de u de test para bolo
...
% Pedimos y separamos los datos de u de training para comida
curs = exec(conn, 'SELECT * FROM u_meal_fir_train');
curs = fetch(curs);
data = curs.Data;
u_meal_delta_train = cell2mat(data(:,2));
u_meal_gaus_train = cell2mat(data(:,3));
u_meal_hov_train = cell2mat(data(:,4));

% Pedimos y separamos los datos de u de test para comida
curs = exec(conn, 'SELECT * FROM u_meal_fir_test');
curs = fetch(curs);
data = curs.Data;
u_meal_delta_test = cell2mat(data(:,2));
u_meal_gaus_test = cell2mat(data(:,3));
u_meal_hov_test = cell2mat(data(:,4));
...
table_name = 'rmse_ARX_1_y0';       % Tabla (1 para cada conjunto de y
new_columns = { 'train_bolo_rem_meal_hov',...
                 'test_bolo_rem_meal_hov'}; % Columnas
...
u1_train = u_bolo_hov_train;
u1_test  = u_bolo_hov_test;

u2_train = u_meal_hov_train;
u2_test  = u_meal_hov_test;
%% Seleccion de base de datos para trabajar

% Extraemos los datos de training
data_train = [y_train, u1_train, u2_train];
fprintf('N de train: %i\n', length(data_train))

% Extraemos los datos del testing
data_test = [y_test, u1_test, u2_test];
fprintf('N de test:  %i\n', length(data_test))
...
for n=1:n_max_order
    ...
    Y_hat_train = ARX_k_stepV2(data_train, theta, [na max(nb)], k);
    Y_hat_test  = ARX_k_stepV2(data_test,  theta, [na max(nb)], k);
