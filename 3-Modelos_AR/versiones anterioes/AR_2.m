...
s0 = dat(:,3);                          % Columna 2 -> datos filtrados

dat = csvread('data\datos_train.csv', 1);   
s0_train = dat(:,3);                        % Columna 2 -> datos raw

dat = csvread('data\datos_test.csv', 1);     
s0_test = dat(:,3);                         % Columna 2 -> datos raw
...
xlabel('n_a')
ylabel('error [mg/dL]')
saveas(gcf, 'figs/AR_2_err_1.pdf')
%% Grafico de theta
figure(2);
...
saveas(gcf, 'figs/AR_2_theta.pdf')
%% Modelo AR obtenido
...
saveas(gcf, 'figs/AR_2_time_serie.pdf')
...
ylim([-5 5])
grid minor
saveas(gcf, 'figs/AR_2_error_grafico.pdf')

%% Guardamos los datos
csvwrite('data\AR_2_theta.csv',theta_matrix)
csvwrite('data\AR_2_pred_train.csv', k_step_train_na2)
csvwrite('data\AR_2_pred_test.csv', k_step_test_na2)
